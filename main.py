...
def count_down(count):
    ...
    if count > 0:
        ...
        start_timer()
        cycles = math.floor(reps / 2)
        checks_to_print = ""
        for _ in range(cycles):
            checks_to_print += "✔"
        checkmarks.config(text=checks_to_print)
...
status = Label(text="Timer", bg=YELLOW, fg=GREEN, font=(FONT_NAME, 36, "normal"))
status.grid(column=1, row=0)

start_button = Button(text="Start", font=(FONT_NAME, 8, "normal"), command=start_timer)
start_button.grid(column=0, row=2)

reset_button = Button(text="Reset", font=(FONT_NAME, 8, "normal"), command=reset_timer)
